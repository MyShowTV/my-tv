export default {
  async fetch(request) {
    ...
    // ---------------------------------------------------------
    // 1. 核心配置表：只需在这里更新龙华的“钥匙” (AssetID)
    // ---------------------------------------------------------
    const config = {
      // --- 成都系列 (依然是全自动，不受 IP 影响) ---
      ...
      // --- 龙华系列 (在这里填入你抓到的 AssetID) ---
      "lhtv01": { name: "龙华电影", key: "ztAK5EHzPGE", type: "ofiii" }, // 填入你抓到的最新 ID
    ...
    try {
      ...
      // 龙华逻辑：使用手动钥匙拼接 + 自动路径补全
      if (ch.type === "ofiii") {
        if (ch.key === "这里填钥匙") return new Response("请先在代码中填入该频道的 AssetID 钥匙", { status: 500 });
